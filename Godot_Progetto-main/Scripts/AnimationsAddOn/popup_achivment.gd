...
var testi_achievements = {
	...
	"primoAcquisto": {"titolo": "Biscottoni", "desc": "Hai fatto il tuo primo acquisto nel negozio!","icona": preload("res://Sprites/Achivments/spendaccione.png"), "target": 1},
...
func mostra_popup(id_achievement: String):
	if testi_achievements.has(id_achievement):
		lbl_titolo.text = str(testi_achievements[id_achievement]["titolo"])
