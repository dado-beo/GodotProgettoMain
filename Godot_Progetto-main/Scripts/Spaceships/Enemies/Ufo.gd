...
const WAVE_FREQ: float = 4.0 # Velocità dell'oscillazione dell'onda
const WAVE_AMP: float = 120.0 # Ampiezza dell'onda (quanto va a destra/sinistra)

# Variabili per Cerchio (Circling)
var circle_timer: float = 0.0
const CIRCLE_SPEED: float = 200.0
const CIRCLE_ROT_SPEED: float = 4.0 # Radianti al sec (velocità del giro della morte)
